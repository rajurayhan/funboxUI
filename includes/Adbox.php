...
class Adbox
{
    ...
    {
        $unsubDiff = $this->airtelUnsubscribedWithIn('FB');

        if($unsubDiff == 'NULL' || $unsubDiff>15){  
            //exit('Diff: '.$unsubDiff);          
    ...
    // Airtel Same Day Subscription Issue. Bypass Users to Another Location if last unsub within 15 days.
    public function airtelUnsubscribedWithIn($serviceCode){ // FB
        $this->getDB()->select( 'sms' );
        $sql = sprintf( "SELECT unsubs_date FROM robi_subscribers WHERE msisdn = '%s' AND service = '%s';", $this->getMsisdn(), $serviceCode );
        $result = $this->getDB()->get_row( $sql );

        //$data = 1;

        if($result){
            $unsubDate  =  mb_substr($result->unsubs_date, 0, 10);
            $unsubDate  = date_create($unsubDate);
            $today      = date_create(date('Y-m-d')); 

            $diff=date_diff($today,$unsubDate);
            $diff= $diff->format("%a");
        }
        else{
            $diff = 'NULL';
